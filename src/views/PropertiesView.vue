  ...
  <div class="p-4">
    ...
    <div v-if="isListView" class="mt-4">
      <table class="w-full table-fixed">
        <thead>
          <tr class="bg-gray-600 border rounded-t-xl">
            <th
              class="w-1/4 py-4 px-6 text-left text-gray-200 font-bold uppercase"
            >
              Name
            </th>
            <th
        ...
        <tbody class="bg-white">
          <tr v-for="property in filteredProperties" :key="property.id">
            ...
            <td class="py-4 px-6 border-b border-gray-200">
              <span>{{ property.currency }}</span> {{ property.price }}
            </td>
            <td class="py-4 px-6 border-b border-gray-200">
              <span
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
              <RouterLink
                :to="{ name: 'detail', params: { id: property.id } }"
                class="text-blue-500"
                >View</RouterLink
              >
              <button
    ...
    >
      ...
      >
        ...
        />
        <h2 class="mt-2 text-lg font-semibold">{{ property.title }}</h2>
        ...
        <div class="mt-4 flex justify-between">
          <RouterLink
            ...
            >View</RouterLink
          >
          <button @click="editProperty(property.id)" class="text-yellow-500">
            Edit
          </button>
          <button @click="deleteProperty(property.id)" class="text-red-500">
            Delete
...
// import type { ListBulletIcon, Squares2X2Icon } from "@heroicons/vue/24/outline";
...
const isListView = ref(true); // Toggle between list and card view
...
const deleteProperty = (id: number) => {
  ...
  // propertyStore.deleteProperty(id); // Assuming deleteProperty exists in the store
};

// Filter properties on input
const filterProperties = () => {
  // Logic is handled in the computed property `filteredProperties`
};
</script>
